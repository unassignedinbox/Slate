...
ContentFormatCategory ContentCodec::Classify(const std::string& Path) noexcept
{
    ...
    if (Extension == ".projectspace")               return ContentFormatCategory::FrontierSpace;
...
const char* ContentCodec::NameOf(ContentFormatCategory Format) noexcept
{
    switch (Format)
    {
        case ContentFormatCategory::FrontierSpace: return "Frontier Space";
...
bool ContentCodec::Decode(const std::string& Path, SceneStructure& Out, TextureIndex* Textures, const SceneDecodeConfiguration& Config, std::string* Error) noexcept
{
    ...
    {
        case ContentFormatCategory::FrontierSpace: return SpaceSceneCodec::Decode(Path, Out, Textures, Config, Error);
        ...
        default:
            if (Error) *Error = "unsupported content format '" + std::filesystem::path(Path).extension().string() + "' (expected .projectspace, .gltf, .glb, .fbx or .obj)";
            return false;
    }
}
