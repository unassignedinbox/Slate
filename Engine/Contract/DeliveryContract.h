...
///        re-establishment — a resize that failed would have reported success and ticked on against a chain
///        that no longer existed. A delivery genuinely not worth reading is discarded through
...
struct [[nodiscard]] ContentDelivery
{
    ...
    /// note  🔴 🔍 Under SLATE_DEBUG a read of refused content halts here, at the call that did it. Without
    ///        the check it returns a default-constructed Content — a zero handle, an empty span, an identity
    ///        of generation zero — and the defect surfaces wherever that value is finally used, which is
    ///        never the call that failed to test the delivery.
    /// cost  ✔️
...
/// use   `Disregard(NamingEdge->Declare(…));` — a debug label that the vendor declined changes nothing
///       about the object it would have named.
