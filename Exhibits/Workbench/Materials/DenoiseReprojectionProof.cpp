...
int main()
{
    ...
    else
    {
        ...
        const TextPin Pins[] =
        {
            ...
            // Temporal images are a strict double-buffered ABI: raw/sample-space history is read only from the
            // previous slot and all current writes have independently bound set-1 targets. These pins complement
            // the CPU reactive-policy cases below by making a same-image feedback regression visible without a GPU.
            { "ReSTIRViewport", "layout(rgba32f, binding = 3) uniform readonly image2D PreviousHistoryImage;", 1u, "B13a raw history is a readonly previous-frame shader input" },
            { "ReSTIRViewport", "layout(set = 1, binding = 0, rgba32f) uniform writeonly image2D CurrentHistoryImage;", 1u, "B13b raw history has a writeonly current-frame target" },
            { "ReSTIRViewport", "layout(set = 1, binding = 4, rgba32f) uniform readonly  image2D PreviousFilteredHistoryImage;", 1u, "B13c filtered feedback is also previous-frame only" },
            { "ReSTIRViewport", "imageStore(CurrentHistoryImage,     ivec2(pixel), vec4(mean, count));", 1u, "B13d the temporal mean writes only the current target" },
            { "SwapchainExchange", "const uint32_t PreviousSlot = CurrentSlot ^ 1u;", 1u, "B13e host descriptor writer pairs current slot N with previous N xor 1" },
            { "SwapchainExchange", "Vulkan->ComputeDescriptorSets[Vulkan->ActiveSlot]", 1u, "B13f dispatch binds the active slot's immutable set 0" },
            { "SwapchainExchange", "Vulkan->ComputeTemporalDescriptorSets[Vulkan->ActiveSlot]", 1u, "B13g dispatch binds the active slot's immutable temporal set 1" },
            { "SwapchainExchange", "DenoiseSets[Vulkan->ActiveSlot][Level]", 1u, "B13h every wavelet level uses the active slot's descriptor set" },
            { "ReSTIRViewport", "const uint kFeatureSpatialMaterialCompatibility = 1024u;", 1u, "B13i spatial material/roughness bit matches the engine" },
            { "ReSTIRViewport", "const uint kFeatureSpatialObjectIdentity        = 2048u;", 1u, "B13j spatial object identity bit matches the engine" },
            { "ReSTIRViewport", "bool SpatialReuseCompatible(GpuReservoir neighbour, uint receiverIdentity, float receiverRoughness)", 1u, "B13k spatial compatibility is one common direct/GI policy" },
            { "ReSTIRViewport", "&& SpatialReuseCompatible(neigh, primaryIdentity,", 2u, "B13l both direct and GI spatial taps apply the A/B policy" },
