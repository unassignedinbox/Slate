...
// 33. regular polygons use the same bounded, per-edge band builder as rectangles and slots.
{ if(M.anyTool&&M.anyTool())M.selectTool();M.newDoc({silent:true});M.startOp('polygon');M.toolClick(480,350);M.toolClick(560,350);M.finishTool();const pcurve=M.doc.figures.find(f=>f.ctype==='poly');const psk=M.byId(pcurve.parent);M.runCmd('extrude '+psk.name+' 18');const pb=M.doc.figures.find(f=>f.op==='extrude');pb.edits=[{type:'chamfer',key:'top:2',r:2}];M.invalidate(pb.id);M.topoCache.clear();const pm=M.meshOf(pb),pt=M.topo(pb);ok(M.brepAudit(pm).ok,'polygon extrusion with one chamfer is a closed B-rep');ok(pt.faces.some(f=>f.key==='ctop:2'&&f.kind==='plane')&&!pt.faces.some(f=>/^f?top:(0|1|3|4|5)$/.test(f.key)&&f.key!=='ctop:2'),'polygon chamfer stays on the selected top edge');M.doc.sel.clear();M.sub_.sel.clear(); }
