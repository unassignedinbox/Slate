...
This writes `Projects/Project-Zero/Diagnostics/ProjectZero_Showcase.png`; the reference run is 1280×720, 12 bounce candidates and four spatial passes. The latest run completed in 146.4 seconds and produced SHA-256 `139cc7e6485045e71363cf103ceaa0943afbfeb6e0e84b5ec1715272b613c247`. The CPU authored path now continues the exact `MaterialEvaluation.slang` BSDF through smooth-normal reflected/refracted secondary rays, samples the grid luminaire and sky-backed transmission, and applies a five-level normal/depth-aware à-trous filter, so metals and glass are not reduced to the old Lambert/albedo fallback.
