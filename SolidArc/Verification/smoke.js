...
{ if(M.anyTool&&M.anyTool())M.selectTool();M.view.target=[0,0,0];M.view.dist=260;M.setView('top');M.resize();
  ...
  M.topoCache.clear();const ti=M.topo(b).edges.findIndex(e=>e.tangent&&!/^(top|bot):/.test((e.key||'').replace(/^h\d+\./,'')));ok(ti>=0,'a true tangent joint exists');M.sub_.sel.clear();M.sub_.sel.set('k',{fig:b.id,kind:'edge',idx:ti});ok(M.solidEditApply(M.solidHitsFromSel(),'fillet',5)===0,'true tangent joints (band boundary / wall joint) are ignored');
...
// 31. per-edge bevel independence: radii/types coexist, re-edit touches one edge, no whole-loop surprise
{ if(M.anyTool&&M.anyTool())M.selectTool();M.view.target=[0,0,0];M.view.dist=260;M.setView('top');M.resize();
  const K=p=>p.map(v=>v.toFixed(3)).join(',');const openEdges=(m)=>{const c=new Map();M.allTris(m).forEach(t=>{const A=(a,b)=>{const k=[K(a),K(b)].sort().join('|');c.set(k,(c.get(k)||0)+1);};A(t[0],t[1]);A(t[1],t[2]);A(t[2],t[0]);});return [...c.values()].filter(v=>v!==2).length;};
  const zrange=(m,fk)=>{const qs=m.quads.filter(q=>q.fkey===fk);const zs=[];qs.forEach(q=>q.p.forEach(p=>zs.push(p[2])));return [Math.min(...zs),Math.max(...zs)];};
  M.startOp('rect');M.toolClick(380,300);M.toolClick(520,380);const R=M.doc.figures.filter(f=>f.kind==='curve').pop();M.selectTool();M.doc.sel=new Set([R.id]);M.startOp('extrude');M.solidKey({key:'3'});M.solidKey({key:'0'});M.solidKey({key:'Enter'});const b=M.doc.figures.filter(f=>f.kind==='body').pop();
  const fil=(key,r,type)=>{M.topoCache.clear();const i=M.topo(b).edges.findIndex(e=>e.key===key);M.sub_.sel.clear();M.sub_.sel.set('k',{fig:b.id,kind:'edge',idx:i});return M.solidEditApply(M.solidHitsFromSel(),type||'fillet',r);};
  ok(fil('top:1',3)===1,'bevel edge top:1 r=3');ok(fil('top:2',8)===1,'bevel edge top:2 r=8');
  M.topoCache.clear();let m=M.meshOf(b);ok(openEdges(m)===0,'two radii watertight');
  const z1=zrange(m,'ftop:1'),z2=zrange(m,'ftop:2');
  ok(Math.abs(z1[0]-27)<1e-6&&Math.abs(z1[1]-30)<1e-6,'top:1 kept r=3 (band 27..30), got '+z1);
  ok(Math.abs(z2[0]-22)<1e-6&&Math.abs(z2[1]-30)<1e-6,'top:2 kept r=8 (band 22..30), got '+z2);
  b.edits=[];M.invalidate(b.id);ok(fil('top:0',3)===1,'fillet top:0');ok(fil('top:1',3,'chamfer')===1,'chamfer top:1');
  M.topoCache.clear();m=M.meshOf(b);
  ok(m.brep.faces.some(f=>f.key==='ftop:0'),'fillet band survives alongside chamfer');
  ok(m.brep.faces.some(f=>f.key==='ctop:1'),'chamfer band exists');ok(openEdges(m)===0,'mixed band watertight');
  M.topoCache.clear();const ei=M.topo(b).edges.findIndex(e=>e.key==='top:0');M.sub_.sel.clear();M.sub_.sel.set('k',{fig:b.id,kind:'edge',idx:ei});
  ok(M.solidCurrentR(b,M.solidHitsFromSel())===3,'re-edit starts from the existing radius');
  ok(fil('top:0',6)===1,'re-edit tangent cap edge top:0 to r=6');
  M.topoCache.clear();m=M.meshOf(b);
  ok(Math.abs(zrange(m,'ftop:0')[0]-24)<1e-6,'top:0 moved to r=6, got '+zrange(m,'ftop:0'));
  ok(m.brep.faces.some(f=>f.key==='ctop:1'),'chamfer neighbour untouched by the re-edit');
  ok(!m.brep.faces.some(f=>f.key==='ftop:2'||f.key==='ftop:3'),'no other edge picked up the bevel');
  M.topoCache.clear();const fi=M.topo(b).faces.findIndex(f=>f.key==='ctop:1');
  M.sub_.sel.clear();M.sub_.sel.set('k',{fig:b.id,kind:'face',idx:fi});
  const keys=M.solidEditKeys(M.solidHitsFromSel()).get(b.id);
  ok(keys&&keys.size===1&&keys.has('top:1'),'clicking the chamfer band selects base edge top:1');
  b.edits=[{type:'fillet',key:'top:all',r:4}];M.invalidate(b.id);M.topoCache.clear();
  ok(M.meshOf(b).brep.faces.filter(f=>/^ftop:/.test(f.key)).length===4,'top:all bevels the whole loop');
  ok(fil('top:1',7)===1,'explicit edge added on top of :all');
  M.topoCache.clear();m=M.meshOf(b);
  ok(Math.abs(zrange(m,'ftop:1')[0]-23)<1e-6,'explicit top:1 uses r=7 over :all');
  ok(Math.abs(zrange(m,'ftop:0')[0]-26)<1e-6,'other edges stay at :all r=4');
  M.sub_.sel.clear();M.doc.sel.clear(); }


// 32. one B-rep kernel: box / cylinder / sketch-extrude are analytic solids with filletable edges
{ if(M.anyTool&&M.anyTool())M.selectTool();M.view.target=[0,0,0];M.view.dist=260;M.setView('top');M.resize();
  const K=p=>p.map(v=>v.toFixed(3)).join(',');const openEdges=(m)=>{const c=new Map();M.allTris(m).forEach(t=>{const A=(a,b)=>{const k=[K(a),K(b)].sort().join('|');c.set(k,(c.get(k)||0)+1);};A(t[0],t[1]);A(t[1],t[2]);A(t[2],t[0]);});return [...c.values()].filter(v=>v!==2).length;};
  M.runCmd('box 40 30 20');const bx=M.doc.figures.filter(f=>f.op==='box').pop();
  const mb=M.meshOf(bx);ok(mb.brep&&mb.brep.faces.length===6&&mb.brep.edges.length===12&&mb.brep.verts.length===8,'box is a 6-face / 12-edge / 8-vertex B-rep');
  ok(Math.abs(M.measure(bx).v-40*30*20)<1e-6,'pristine box volume is analytic');
  const filB=(key,r,type)=>{M.topoCache.clear();const i=M.topo(bx).edges.findIndex(e=>e.key===key);M.sub_.sel.clear();M.sub_.sel.set('k',{fig:bx.id,kind:'edge',idx:i});return M.solidEditApply(M.solidHitsFromSel(),type||'fillet',r);};
  ok(filB('top:0',4)===1,'fillet on a box edge');M.topoCache.clear();
  ok(M.meshOf(bx).brep.faces.some(f=>f.key==='ftop:0'),'box fillet band exists');ok(openEdges(M.meshOf(bx))===0,'filleted box watertight');
  ok(M.measure(bx).v<40*30*20,'edited box volume is tessellated (smaller)');
  M.runCmd('cylinder 10 30');const cy=M.doc.figures.filter(f=>f.op==='cylinder').pop();
  const mc=M.meshOf(cy);ok(mc.brep&&mc.brep.faces.length===3&&mc.brep.edges.length===2&&mc.brep.verts.length===0,'cylinder is a 3-face / 2-edge B-rep');
  const h0=+cy.params.h;M.topoCache.clear();const ti=M.topo(cy).faces.findIndex(f=>f.key==='cap:top');
  M.faceOpApply([M.faceInfoWorld(cy,ti)],'push',5);ok(+cy.params.h===h0+5&&!(cy.faceOps||[]).length,'plain cylinder cap push edits h');
  const filC=(key,r)=>{M.topoCache.clear();const i=M.topo(cy).edges.findIndex(e=>e.key===key);M.sub_.sel.clear();M.sub_.sel.set('k',{fig:cy.id,kind:'edge',idx:i});return M.solidEditApply(M.solidHitsFromSel(),'fillet',r);};
  ok(filC('top:0',3)===1,'fillet on a cylinder edge');M.topoCache.clear();ok(openEdges(M.meshOf(cy))===0,'filleted cylinder watertight');
  const sk2=M.mk('sketch','SketchClean',{},{});const q2=M.mk('curve','QClean',{pts:[[0,0],[50,0],[50,30],[0,30]],closed:true},{ctype:'poly',parent:sk2.id});sk2.children=[q2.id];M.doc.figures.push(sk2,q2);
  M.runCmd('extrude SketchClean 10');
  const ex=M.doc.figures.filter(f=>f.op==='extrude'&&!f.profile).pop();
  ok(ex&&M.meshOf(ex).brep,'sketch extrude has a B-rep');ok(openEdges(M.meshOf(ex))===0,'sketch extrude watertight');
  ok(Math.abs(M.measure(ex).v-50*30*10)<50*30*10*0.01,'sketch extrude volume is analytic-ish');
  M.topoCache.clear();const ei2=M.topo(ex).edges.findIndex(e=>e.key==='top:0');M.sub_.sel.clear();M.sub_.sel.set('k',{fig:ex.id,kind:'edge',idx:ei2});
  ok(M.solidEditApply(M.solidHitsFromSel(),'chamfer',2)===1,'chamfer on a sketch-extrude edge');M.topoCache.clear();ok(openEdges(M.meshOf(ex))===0,'chamfered sketch extrude watertight');
  M.sub_.sel.clear();M.doc.sel.clear(); }
